...
# config_file = 'configs/pascal_voc/cascade_rcnn_x101_64x4d_fpn_1x_crop.py'
config_file="configs/cascade_rcnn_x101_64x4d_fpn_1x.py"
# checkpoint_file = 'work_dirs/cascade_rcnn_x101_64x4d_fpn_1x_crop/epoch_24.pth'  # leakage model.
...
#     show_result(imgs[i], result, model.CLASSES, out_file='results/crop/{}'.format(files[i]))
...
def predict_with_txt(txt_name):
    ...
    for i, result in enumerate(inference_detector(model, imgs)):
        show_result(imgs[i], result, model.CLASSES,
                    out_path='/data2/yeliang/data/tunnel_camera/20190719_line10_camera/Camera7/',
                    out_file='{}'.format(files[i][:-3] + 'jpg'),
                    txt=txt_name)


txt_name = '/data2/yeliang/data/tunnel_camera/20190719_line10_camera/Camera7'
